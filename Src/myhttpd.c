...
/* Private macro
-------------------------------------------------------------*/
void http_log(const unsigned char*data, size_t len)
{
    int i;
    ...
    if(recv_err == ERR_OK)
    {
        if(netconn_err(conn) == ERR_OK)
        {
            ...
            if((buflen >=5) && (strncmp(buf, "GET /", 5) == 0))
            {
                ...
                else if(strncmp((char const *)buf, "GET /system", 11) == 0)
                {
                    pSys = getSysInfoP();
                    pIp = getIpInfoP();
                    
                    file = fs_open_ajax("/system");
                    sprintf(JSON_BUFFER, "%s{\"name\":\"%s\",\"ip\":\"%d.%d.%d.%d\"}",   
                            file->data,       
                            pSys->modelName,                
                            pIp->ip[0], 
                            pIp->ip[1], 
                    ...
                    http_log((const unsigned char*)(JSON_BUFFER), strlen(JSON_BUFFER));
                    
                    netconn_write(conn, (const unsigned char*)(JSON_BUFFER), 
                    (size_t)strlen(JSON_BUFFER),
                        NETCONN_NOCOPY);
                    fs_close(file);
                }
                else
    ...
    if(conn!= NULL)
    {
        ...
        if(err == ERR_OK)
        {
            ...
            while(1)
            {
                ...
                {
                    printf("netconn_accept() failed %d", accept_err);
                }
            }
        }
        else
        {
            printf("netconn_bind fail\n");
        }
    }
    else
    {
        printf("new netconn() fail\n");
    }
}
